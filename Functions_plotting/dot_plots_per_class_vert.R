dot.plots.per.class.vert <-  function(dat_plot, set,  p_effect = 0.8, plot_what = "Healthy" ,sel_effect = F, dvc = "png" ) {
  ...
  pp <- sapply(classes , function(cls) {
    ...
    if(plot_what == "Both") {
      p1 <- p1 +  facet_grid(Class~Facet_vert,scales = "free", space = "free") + coord_flip()
    } else {
      p1 <- p1 +  facet_grid(Class~.,scales = "free", space = "free")  + coord_flip()
    }
